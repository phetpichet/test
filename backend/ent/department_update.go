...
func (du *DepartmentUpdate) SetDname(s string) *DepartmentUpdate {
...
func (du *DepartmentUpdate) Save(ctx context.Context) (int, error) {
	if v, ok := du.mutation.Dname(); ok {
		if err := department.DnameValidator(v); err != nil {
			return 0, &ValidationError{Name: "dname", err: fmt.Errorf("ent: validator failed for field \"dname\": %w", err)}
		}
	}
	var (
		err      error
...
// SetDname sets the dname field.
func (duo *DepartmentUpdateOne) SetDname(s string) *DepartmentUpdateOne {
