...
// The WorkingFunc type is an adapter to allow the use of ordinary
// function as Working mutator.
type WorkingFunc func(context.Context, *ent.WorkingMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WorkingFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.WorkingMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WorkingMutation", m)
	}
	return f(ctx, mv)
}
